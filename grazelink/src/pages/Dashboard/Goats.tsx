...
import BulkAddGoatsModal from '@/components/Dashboard/BulkAddGoatsModal';
...
export default function Goats() {
  ...
  const openEdit = (goat: Goat) => {
    setEditingGoat(goat);
    setModalOpen(true);
  };

  const openAnalyze = (goat: Goat) => {
    navigate(`/dashboard/goats/${goat.id}?view=analytics`);
  };

  const openProfile = (goat: Goat) => {
    navigate(`/dashboard/goats/${goat.id}?view=profile`);
  };

  const handleDelete = async (goat: Goat) => {
    if (!user) return;
    ...
    try {
      ...
      showToast('error', 'Could not delete goat record.');
    }
  };
  ...
  const filteredGoats = useMemo(() => {
    return goats
      .filter((g) => {
        ...
        const matchesSearch =
          g.goatId.toLowerCase().includes(query) ||
      ...
      .sort((a, b) => {
        if (sortBy === 'name') return (a.name || a.goatId).localeCompare(b.name || b.goatId);
        if (sortBy === 'age') return b.age - a.age;
        if (sortBy === 'weight') return b.weight - a.weight;
        if (sortBy === 'battery') return (b.battery ?? 0) - (a.battery ?? 0);
        return a.goatId.localeCompare(b.goatId);
      });
  }, [goats, search, healthFilter, vaccineFilter, sortBy]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <FiPlus className="text-lg" /> Add Goat
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-black/5 bg-white p-4 shadow-soft dark:border-white/5 dark:bg-dark-card dark:shadow-dark-card">
        ...
        <select
          ...
          className="rounded-xl border border-black/10 bg-surface-light px-3 py-2 text-sm font-medium text-ink outline-none focus:border-primary dark:border-white/10 dark:bg-dark-surface dark:text-white"
        >
          <option value="id">Sort by Goat ID</option>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Click below to add your first goat and attach a smart collar to begin tracking."
          action={{ label: '+ Add Goat', onClick: openAdd }}
        />
        ...
        <div className="rounded-2xl border border-black/5 bg-white p-12 text-center shadow-soft dark:border-white/5 dark:bg-dark-card">
          <p className="text-muted dark:text-dark-muted">
            No goats match your search or filters.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="flex items-center justify-between border-t border-black/5 pt-4 dark:border-white/5">
            <span className="text-xs font-medium text-muted dark:text-dark-muted">
              {filteredGoats.length === goats.length
                ? `Showing all ${goats.length} goats`
                : `${filteredGoats.length} of ${goats.length} goats match your search`}
            </span>
          </div>
        </>
      )}
      ...
      {/* Modal */}
      <AddGoatModal open={modalOpen} onClose={() => setModalOpen(false)} goat={editingGoat} />
      <BulkAddGoatsModal open={bulkOpen} onClose={() => setBulkOpen(false)} />
    </div>
  );
}
